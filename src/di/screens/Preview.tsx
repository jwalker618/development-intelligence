...
export function PreviewScreen({
  ...
  const [server, setServer] = useState<string | null>(null); // null = no dev server (honest default)
  const [pick, setPick] = useState(false);
  const vp = vpLocal;
  const setVp = (v: Viewport) => { setVpLocal(v); onViewport(v); };
  const frame = VP.find((v) => v.id === vp)!;
  const SERVERS = [{ port: ":5173", label: "Vite · web-app" }, { port: ":6006", label: "Storybook" }, { port: ":3001", label: "API" }];
  return (
    <>
      {/* ── rail ── */}
      <div style={{ position: "absolute", top: 52, left: 0, width: 340, boxSizing: "border-box", padding: "0 14px", zIndex: 5 }}>
        ...
        <div style={{ position: "relative", marginBottom: 8 }}>
          <button className="di-btn" onClick={() => setPick((p) => !p)} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", height: 34, padding: "0 11px", border: "1px solid var(--di-rail-row-border)", borderRadius: 9, background: "var(--di-rail-row-bg)", cursor: "pointer" }}>
            <span style={{ width: 8, height: 8, borderRadius: 999, background: server ? "var(--di-pos)" : "#586573", flex: "0 0 auto" }} />
            <span className="di-mono" style={{ fontSize: 11, color: "var(--di-ink-mute)", flex: 1, textAlign: "left", minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{server ? `localhost${server}` : "no server"}</span>
            <Icon name="chevron-down" size={13} color="var(--di-ink-mute)" />
          </button>
          {pick && (
            <>
              <div onClick={() => setPick(false)} style={{ position: "fixed", inset: 0, zIndex: 20 }} />
              <div className="di-menu" style={{ position: "absolute", top: 38, left: 0, width: 280, zIndex: 21, border: "1px solid #2c5075", borderRadius: 12, background: "#0a1a2a", boxShadow: "0 30px 70px -18px rgba(0,0,0,.85)", padding: 9 }}>
                <div className="di-eyebrow" style={{ padding: "5px 6px 8px", fontSize: 9 }}>{SERVERS.length} servers detected</div>
                {SERVERS.map((sv) => (
                  <button key={sv.port} className="di-row di-btn" onClick={() => { setServer(sv.port); setPick(false); }} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 12px", borderRadius: 9, border: 0, background: server === sv.port ? "#12283f" : "transparent", cursor: "pointer", marginBottom: 2 }}>
                    <span style={{ width: 8, height: 8, borderRadius: 999, background: "var(--di-pos)" }} /><span className="di-mono" style={{ fontSize: 12, color: "var(--di-ink)", flex: 1, textAlign: "left" }}>{sv.port}</span><span style={{ fontSize: 10.5, color: "#6f8296" }}>{sv.label}</span>
                    {server === sv.port && <Icon name="check" size={14} color="var(--di-info)" />}
                  </button>
                ))}
                <div className="di-btn" style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px", borderTop: "1px solid #17293a", marginTop: 4, cursor: "pointer" }}><Icon name="plus" size={13} color="var(--di-info)" /><span style={{ fontSize: 11.5, color: "var(--di-info)" }}>Add a port manually</span></div>
              </div>
            </>
          )}
        </div>
      ...
      <div className="di-scroll" style={{ position: "absolute", top: 178, left: 0, width: 340, bottom: 56, boxSizing: "border-box", padding: "0 14px", zIndex: 5 }}>
        ...
        {!server ? (
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 11px", border: "1px solid var(--di-rule)", borderRadius: 10, background: "var(--di-surface-sunken)" }}>
            <Icon name="minus-circle" size={14} color="var(--di-ink-mute)" /><span style={{ fontSize: 11.5, color: "#6f8296" }}>nothing to inspect</span>
          </div>
        ) : (
      ...
      <MainColumn style={{ background: "radial-gradient(130% 90% at 50% -10%,#123152 0%,#0a1f33 45%,#07131f 100%)", alignItems: "center", justifyContent: "center" }}>
        ...
        {server ? <DeviceFrame w={frame.w} h={frame.h} phone={vp === "phone"} /> : (
          <div style={{ textAlign: "center", maxWidth: 420 }}>
            <div style={{ width: 62, height: 62, margin: "0 auto 18px", borderRadius: 15, background: "rgba(10,20,32,.7)", border: "1px solid var(--di-rule)", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="monitor-off" size={28} color="var(--di-ink-mute)" /></div>
            <div style={{ fontSize: 18, fontWeight: 600, color: "var(--di-ink)", letterSpacing: "-0.01em", marginBottom: 8 }}>No dev server running</div>
            <div style={{ fontSize: 13, color: "var(--di-ink-mute)", lineHeight: 1.55, marginBottom: 20 }}>Start your dev server and the running app appears here — phone, tablet, or desktop.</div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "8px 8px 8px 14px", border: "1px solid var(--di-rule)", borderRadius: 12, background: "rgba(10,20,32,.7)" }}>
              <span className="di-mono" style={{ fontSize: 12, color: "#a9bccf" }}>pnpm dev</span>
              <button className="di-btn" onClick={() => setServer(":5173")} style={{ height: 34, padding: "0 15px", border: 0, borderRadius: 9, background: "var(--di-spot)", color: "#3a140a", fontFamily: "inherit", fontSize: 12.5, fontWeight: 600, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 7 }}><Icon name="play" size={14} />Start</button>
            </div>
          </div>
        )}
      </MainColumn>
